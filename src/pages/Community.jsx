...
import { Calendar, User, MapPin, Compass } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-border rounded-3xl bg-secondary/20">
          ...
          <h3 className="font-display text-xl font-semibold mb-2">{t('community.empty')}</h3>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sharedTrips.map((trip, idx) => (
            ...
            >
              <Link to={`/community/trip/${trip.id}`} className="group block h-full">
                <div className="relative h-full flex flex-col rounded-2xl overflow-hidden border border-border bg-card hover:shadow-xl hover:shadow-primary/5 transition-all duration-300">
                  {/* Image */}
                  <div className="relative h-48 w-full overflow-hidden bg-secondary">
                    ...
                    <div className="absolute top-4 left-4">
                        <Badge className="bg-white/90 text-black backdrop-blur-sm border-none shadow-sm capitalize">
